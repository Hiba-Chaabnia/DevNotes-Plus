...
import { countNoteAndMaybePrompt, hasStarred } from '../services/Support';
...
export class SidebarView implements vscode.WebviewViewProvider {
  ...
  private async _flush(): Promise<void> {
    ...
    this.view.webview.postMessage({
      ...
      starred           : hasStarred(this.context),
    });
  }
  ...
  private async handle(msg: ToExt): Promise<void> {
    switch (msg.type) {
      ...
      case 'createNote': {
        ...
        void countNoteAndMaybePrompt(this.context);
        this.push();
        break;
      }
      ...
      case 'starRepo':
        vscode.commands.executeCommand('devnotesPlus.starOnGitHub');
        break;
...
<div class="overflow-menu" id="overflow-menu">
  ...
  <button class="ovf-item" id="btn-star-repo">
    <span class="ovf-icon">${svgIcon(ALL_LUCIDE_NODES['Star'], 14)}</span>
    <span class="ovf-label">Star on GitHub</span>
  </button>
</div>
...
(() => {
  ...
  btnStarRepo.addEventListener('click', () => {
    vscode.postMessage({ type: 'starRepo' });
    overflowMenu.classList.remove('open');
  });
  ...
  window.addEventListener('message', ({ data: msg }) => {
    ...
    if (msg.type === 'init') {
      ...
      btnStarRepo.style.display = msg.starred ? 'none' : '';
